...
export class S3Service {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.privateBucket = this.configService.getOrThrow("AWS_PRIVATE_BUCKET_NAME");
        this.publicBucket = this.configService.getOrThrow("AWS_PUBLIC_BUCKET_NAME");
    }
    ...
    async upload({ buffer, key, bucketName }: { buffer: Buffer; key: string; bucketName?: string }) {
        try {
            if (!key || !buffer || typeof key !== "string") throw new ApiError(API_ERROR_CODE.NOT_SET_S3_PATH);
    ...
    async retrieveFile({ res, key, bucketName }: { res: Response; key: string; bucketName?: string }) {
        try {
    ...
    async retrieveFileBuffer({ key, bucketName }: { key?: string; bucketName?: string }) {
        try {
            if (!key || typeof key !== "string") throw new ApiError(API_ERROR_CODE.NOT_SET_S3_PATH);
    ...
    async copyFile({
        ...
        try {
            if (!sourceKey || !destinationKey || typeof sourceKey !== "string" || typeof destinationKey !== "string")
    ...
    async getFilesList({ prefix, bucketName }: { prefix: string; bucketName?: string }): Promise<S3DirNode> {
    ...
    getKeyByListObject = ({
        ...
        for (const seg of folderPath) {
            const next = current.children.find((node): node is S3DirNode => node.type === "dir" && node.name === seg);
            if (!next) return null;
            current = next;
        }
        const target = current.children.find(
            (node): node is S3FileNode =>
                node.type === "file" && node.normalize.toLocaleLowerCase() === fileName.normalize().toLocaleLowerCase()
        );
