...
export class S3Path {
    private environment = (process.env.LF_ENV as "production" | "staging" | "development") ?? "development";
    private readonly fileExtensionRegexp = /\.[^/.]+$/;
    private path: string[] = [];
    private fileName: string = "";

    constructor({ url, dirArr, id, fileName, base = "upload", isIncludeEnvPath = true }: S3PathParam) {
    ...
    getExtension = (): string => this.fileName.match(this.fileExtensionRegexp)?.[0]?.toLowerCase() || "";
