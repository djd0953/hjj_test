...
const parseExcel = async (filePath: string) => {
    ...
    try {
        ...
        const data: {
            no: number;
            path: string | null;
            title: string | number | true | Date | CellErrorValue;
            att: string | number | true | Date | CellErrorValue;
        }[] = [];
        worksheet.eachRow((row, rowNumber) => {
            ...
            if (Array.isArray(row.values)) {
                ...
                if (typeof val2 === "string") val2 = val2.normalize();
                    ...
                    val2 &&
                    typeof val2 === "object" &&
                    (val2 as exceljs.CellFormulaValue | exceljs.CellSharedFormulaValue)?.result
                )
                    val2 = (val2 as exceljs.CellFormulaValue | exceljs.CellSharedFormulaValue).result ?? "";
                else if (val2 && typeof val2 === "object" && val2 instanceof Date)
                    val2 = dayjs(val2).format("YYYY-MM-DD");
                else if (val2 && typeof val2 === "object" && val2.hasOwnProperty("richText"))
                    val2 = (val2 as exceljs.CellRichTextValue).richText.map((z: exceljs.RichText) => z.text).join("");
                else if (val2 && typeof val2 === "object" && (val2 as exceljs.CellHyperlinkValue)?.text)
                    val2 = (val2 as exceljs.CellHyperlinkValue).text.toString();
                else if (val2 && typeof val2 === "object" && (val2 as exceljs.CellErrorValue)?.error)
                    val2 = (val2 as exceljs.CellErrorValue).error.toString();
                else val2 = "";
...
export const excelFileCheck = async () => {
    ...
    if (excelFilePath) {
        ...
        for (const d of excelData) {
            if (typeof d.title !== "string" && typeof d.att !== "string") continue;

            const match = files.find(
                (f) =>
                    f.title.replace(/\s/g, "").toLocaleLowerCase() ===
                    (d.title as string).replace(/\s/g, "").toLocaleLowerCase()
            );
            if (match) d.path = match.path.split("/").slice(0, -1).join("/").normalize();
            lines.push(`${d.no}\t${d.path}\t${d.title as string}\t${d.att as string}\n`);
        }
