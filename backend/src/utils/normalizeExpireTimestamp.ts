...
const ZERO_PATTERNS = new Set(["0000-00-00", "0000-00-00 00:00:00", ""]);
...
const parseMysqlLiteralToUtc = (s: string): Date | null => {
    ...
    if (!m) return null;
    const [_, Y, M, D, h, m2, s2] = m.map(Number) as unknown as [
        string,
        number,
        number,
        number,
...
export default (input: string | Date | null | undefined): string => {
