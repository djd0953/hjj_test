...
export function hoistTablesOutOfLi(html: string): string {
    const dom = new JSDOM();
    ...
    const allLis: HTMLLIElement[] = Array.from(wrap.querySelectorAll("li"))
        .filter((li) => Array.from(li.querySelectorAll("table")).some((t) => t.closest("li") === li))
        .sort((a, b) => {
            const da = liDepth(a),
                db = liDepth(b);
    ...
    for (const li of allLis) {
        ...
        // 현재 li의 "직속" table만
        const tables = Array.from(li.querySelectorAll("table")).filter((t) => t.closest("li") === li);
        ...
        if (listEl.parentElement && listEl.parentElement.tagName.toLowerCase() !== "li") {
            ...
            if (tailLis.length > 0) {
                const newList = document.createElement(listTag) as HTMLOListElement | HTMLUListElement;
                carryListAttrs(listEl, newList);

                if (listTag === "ol") {
                    const startAttr = parseInt((listEl as HTMLOListElement).getAttribute("start") || "1", 10);
        ...
        if (outerList.tagName.toLowerCase() === "ol") {
            const startAttrOuter = parseInt((outerList as HTMLOListElement).getAttribute("start") || "1", 10);
        ...
        if (tailLis.length > 0) {
            const newInnerList = document.createElement(listTag) as HTMLOListElement | HTMLUListElement;
