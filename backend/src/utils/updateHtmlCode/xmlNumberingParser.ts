...
const _DOCX_NUMBERING_PLAN_ER: Array<
    [fmt: "decimal" | "upperLetter" | "lowerLetter" | "upperRoman" | "lowerRoman" | "ganada" | "chosung", text: string]
> = [
    ["decimal", "%1."], // 1: 1.
    ["upperLetter", "%2."], // 2: A.
    ["lowerRoman", "%3."], // 3: i.
...
function _patchNumberingXml(
    ...
    for (const abs of targets) {
        ...
        for (let ilvl = 0; ilvl < DOCX_NUMBERING_PLAN.length; ilvl++) {
            const lvl = lvls.find(
                (l) => l.getAttributeNS(W, "ilvl") === String(ilvl) || l.getAttribute("w:ilvl") === String(ilvl)
            );
            if (!lvl) continue;
...
function _normalizeListNumIds(documentXml: string) {
    ...
    if (hasParseError) return documentXml;
