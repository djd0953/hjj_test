//@ts-ignore - TS 버전이 npm에 없음 하지만 기존껄로도 사용 가능함
...
type J = {
    inlineContent: (html: string, css: string, opt?: Record<string, unknown>) => string;
};
...
export async function inlineAllCssWithJuiceAndPseudo(html: string): Promise<string> {
    ...
    const root = doc.querySelector(WRAPPER_ID);
    if (!root) {
        iframe.remove();
        return prepared;
    }
...
function copyInlineSubset(src: CSSStyleDeclaration, dst: CSSStyleDeclaration, keys: string[]) {
    for (const k of keys) {
        const v = src.getPropertyValue(camelToKebab(k));
        if (v) dst.setProperty(camelToKebab(k), v);
    }
}
...
function postNormalizeForDocx(html: string): string {
    ...
    const wrap = dom.window.document.body.firstElementChild as HTMLElement;
    ...
    wrap.querySelectorAll<HTMLElement>("*").forEach((el) => {
        ...
        s = s.replace(
            /font-sizes*:s*([d.]+)s*pxs*(!important)?/gi,
            (_: string, pxStr: string, imp: string | undefined) => {
