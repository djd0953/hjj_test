...
const WS_URL = process.env.NEXT_PUBLIC_LEGACY_WS_ORIGIN ?? "http://localhost:9090";
...
export default function BlackjackOnline()
{
    ...
    const connect = useCallback(() =>
    {
        ...
        {
            setConnectionError("legacy Nest 서버에 연결하지 못했습니다. backend에서 npm run dev를 실행해 주세요.");
        });
    }, [myId]);
        ...
        <div className="flex flex-col items-center gap-3 w-full">
            ...
            {!connected ? (
                <div className="flex flex-col items-center gap-2">
                    ...
                    {connectionError ? <p className="m-0 text-sm text-red-600">{connectionError}</p> : null}
                </div>
            ) : (
