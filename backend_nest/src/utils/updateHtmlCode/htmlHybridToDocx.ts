...
type BorderSide = BorderInfo | NilBorder | null;
...
interface MatrixRow extends Array<MatrixCell | null> {
    _trHeight?: number | null;
}

interface TableComputed {
    tblWType: string;
    tblWVal: number;
    collapse: boolean;
    borderInfo: BorderInfo | null;
    paddingDxa: { top: number; right: number; bottom: number; left: number };
    grid: number[];
}

// --- 유틸: 단위/색 변환 ---
const pxToDxa = (px: string | number) => Math.round(Number(px || 0) * 15); // 1px ≈ 0.75pt, 1pt=20dxa → 0.75*20=15
const ptToDxa = (pt: string | number) => Math.round(Number(pt || 0) * 20);
const pct100ToWordPct = (pct: string | number) => Math.round(Number(pct) * 50); // Word: 100% = 5000

const normalizeHex = (c: string | null | undefined) => {
    if (!c) return null;
    const m = String(c)
        .trim()
...
const parseBorder = (val: string): BorderInfo | null => {
    if (!val) return null;
...
const parseBorderSide = (val: string): BorderSide => {
    if (!val) return null;
...
const getCellBordersFromStyle = (style: Record<string, string>): CellBorders | null => {
...
const buildCellMatrixWithStyles = (tableHtml: string) => {
    //@ts-ignore - 2번째 파라미터가 index.d.ts에 없음
    ...
    rows.forEach((r, rowIdx) => {
        ...
        let trHeight: number | null = null;
        if (rowStyle["height"]) {
            const m = rowStyle["height"].match(/([\d.]+)(px|pt)/i);
            if (m) trHeight = m[2].toLowerCase() === "pt" ? ptToDxa(m[1]) : pxToDxa(m[1]);
        }

        if (!matrix[rowIdx]) matrix[rowIdx] = Array.from({ length: colCount }, () => null) as MatrixRow;
        ...
        $(r)
            ...
            .each((_, c) => {
                ...
                for (let rspan = 1; rspan < rowspan; rspan++) {
                    ...
                    if (!matrix[targetRow])
                        matrix[targetRow] = Array.from({ length: colCount }, () => null) as MatrixRow;
    ...
    let grid: number[] = [];
    const firstRow: MatrixRow = matrix[0] || ([] as MatrixRow);
    const hasAnyWidth = firstRow.some((c) => c?.cellStyle?.tcW);
    if (hasAnyWidth) {
    ...
    const probe = firstRow.find((c) => c?.cellStyle?.paddings);
...
// --- 매트릭스 → DOCX 테이블 XML ---
const matrixToTableXmlWithStyles = (matrix: MatrixRow[], tableComputed: TableComputed) => {
    const { tblWType, tblWVal, collapse, borderInfo, paddingDxa, grid } = tableComputed;

    const bordersXml = borderInfo
        ? `
    <w:tblBorders>
      <w:top w:val="${borderInfo.val}" w:sz="${borderInfo.sz}" w:space="0" w:color="${borderInfo.color}"/>
      <w:left w:val="${borderInfo.val}" w:sz="${borderInfo.sz}" w:space="0" w:color="${borderInfo.color}"/>
      <w:bottom w:val="${borderInfo.val}" w:sz="${borderInfo.sz}" w:space="0" w:color="${borderInfo.color}"/>
      <w:right w:val="${borderInfo.val}" w:sz="${borderInfo.sz}" w:space="0" w:color="${borderInfo.color}"/>
    ...
    const gridXml = `
    <w:tblGrid>
      ${grid.map((w: number) => `<w:gridCol w:w="${w}"/>`).join("")}
    </w:tblGrid>`;

    const sideXml = (name: string, info: BorderSide) => {
        if (!info) return "";
        if ("nil" in info) return `<w:${name} w:val="nil"/>`;
    ...
    const cellXml = (cell: MatrixCell | null): string => {
        ...
        const pad = st.paddings || { top: 0, right: 0, bottom: 0, left: 0 };
        const tcMar = `<w:tcMar>
