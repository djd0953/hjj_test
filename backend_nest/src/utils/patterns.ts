...
export const Patterns = {
    ...
    dateTime:
        /^([0-2][0-9]{3})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])\s(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])(?:([+-](0[0-9]|1[0-4])):00)?$/,
...
export class PatternValidator {
    ...
    public static isValidDate(input: string): boolean {
        ...
        if (!Patterns.dateYmd.test(trimmed)) return false;
    ...
    public static isValidDateTime(input: string): boolean {
        ...
        if (!Patterns.dateTime.test(trimmed)) return false;
    ...
    public static isValidIpV4WithPort(input: string): boolean {
        ...
        // 1차: 정규식 매치
        if (!Patterns.ipv4WithOptionalPort.test(trimmed)) return false;
