...
const S3ListObjects = async ({ key }) => {
    ...
    try {
        ...
        while (isTruncated) {
            ...
            const data = new client_s3_1.ListObjectsV2Command(params);
...
const settingEmailJson = async (num) => {
    ...
    for await (const path of (num !== undefined ? [paths[num]] : paths)) {
        ...
        r.push({ text, html, from, to, subject, messageId, references, inReplyTo, textAsHtml });
    }
...
const parsingExcelData = async () => {
    ...
    const dataModel = dataSheet?.model;
    let headers = [];
    const values = [];
    for (const row of dataModel.rows) {
        const cells = row.cells.map(x => {
            ...
            if (typeof x.value === "object" && x.value?.richText)
                ...
                val = (0, dayjs_1.default)(x.value).format('YYYY.MM.DD HH:mm:ss');
            else if (x.result)
                val = x.result;
            return {
    ...
    const r = values.map(x => {
        ...
        for (const header of headers) {
            ...
            obj[key] = x.find(cell => cell.address === address)?.value || '';
        }
        return obj;
    });
    return r;
...
const diffDate = (text) => {
    ...
    const startDate = (0, dayjs_1.default)(`${start.y}.${start.m}.${start.d}`);
    const endDate = (0, dayjs_1.default)(`${end.y}.${end.m}.${end.d}`);
...
const checkMissingFiles = async (data) => {
    ...
    const missingFiles = await Promise.all(data.map(async (item) => {
        const filePath = !item.contractCategoryName ? path_1.default.join(defaultPath, item.teamName, item.cfsFileName) : path_1.default.join(defaultPath, item.teamName, item.contractCategoryName, item.cfsFileName);
        try {
    ...
    await fs_1.default.writeFileSync("missing_files.txt", missingFiles.filter(v => v !== null).join('\n'), { encoding: 'utf-8' });
};
const test = () => {
    const a = [
        {
...
const getTreeToS3List = (list, prefix) => {
    const tree = [];
...
const start = async () => {
    ...
    // const c2 = dayjs()
    // console.log(c > c2)
    const key = `business_partners_files/deaju/migration_files/contract/구매/포장재/55-1. 피앤비_NDA.pdf`;
    const buffer = await S3RetreiveFileBuffer({ key });
    fs_1.default.writeFileSync('tt.pdf', key);
    console.log(1);
    console.log(1);
};
start();
