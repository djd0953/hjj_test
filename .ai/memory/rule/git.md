...
## 🟠 사용자 별도 요청 시 허용 — `git commit`

**커밋은 코드 작업과 별개다.** agent는 구현·검증을 마쳤더라도 자동으로 커밋하지 않으며, 사용자가 `커밋해줘`처럼 명시적으로 요청한 경우에만 수행한다. `플랜 적용해줘`, 구현 요청, 수정 요청은 커밋 승인이 아니다.

명시 요청이 있더라도 아래 형식과 절차를 지킬 때만 커밋한다.
...
- **커밋은 사용자 별도 요청 때만, 원격 반영은 금지.** 로컬 커밋도 사용자가 결정하고, 밖으로 나가는 것은 어떤 경우에도 agent가 실행하지 않는다
