...
## 2026-08-31 — 구조 재설계: 기능 우선 패키지 + 확장 가능한 라이브러리 모듈

- 사용자가 NestJS의 `modules/{기능}/{controllers,services,dto,...}` 탐색 경험을 기준으로 현재 구조를 재검토했다.
  기존 `usecase/{controller,service}/{도메인}`과 최상위 `auth`/`config`/`interceptor`/`type` 분산은
  로그인처럼 하나의 엔드포인트에서 문제를 추적할 때 여러 루트를 왕복하게 한다는 결론.
- **패키지 전략을 package-by-feature로 번복·확정**: `hjj.authentication/**`, `hjj.code/**`처럼 기능을 먼저 찾고
  그 안에서 controller/service/model/request 등 역할로 나눈다. 전역 HTTP 오류는 `hjj.web.error`, 순수 공용 코드는
  `hjj.common/{주제}`로 둔다. `auth`라는 이름 아래에 데이터 클래스만 있어 기능 전체가 보이지 않던 문제를 해소한다.
- 현재 `shared + api` 구조를 장기적으로 **`core` / `persistence` / `infrastructure-aws` / 실행 앱들**로 확장하기로 했다.
  `core`는 순수 타입·계약만, `persistence`는 DB 구현, `infrastructure-aws`는 AWS SDK 구현을 맡는다.
  구현체를 api/scheduler/thirdparty에 복사하지 않고 전용 인프라 모듈 하나를 공유한다.
- 학습 목적상 지금 `core` 전환과 `persistence`·`infrastructure-aws` 골격까지 만든다. 단 실제 책임이 없는
  scheduler/thirdparty Boot 앱은 만들지 않는다. JPA/AWS를 시작할 때 각 골격을 채운다.
- 이 결정과 이동표·Gradle 의존 방향은 `spec.md`와 `plan.md` 청크 0에 기록. 인증 구현은 먼저 현재 컴파일을 통과시킨 뒤
  구조만 이동하여, 기능 오류와 리팩터링 오류를 분리해 검증한다.

## 2026-08-31 — 모듈 단순화: `core + infrastructure + 실행 앱`

- 외부 라이브러리 하나가 늘 때마다 `persistence`, `infrastructure-aws`처럼 Gradle 모듈을 늘리는 방식은
  탐색성을 떨어뜨리고 사용자 선호와 맞지 않는다고 재검토했다. DB도 본질적으로는 외부 시스템과 연결하는 구현이라는
  관점에 합의했다.
- 장기 구조를 **`core` / `infrastructure` / 실행 앱들**로 단순화한다. `infrastructure` 안을
  `persistence/jpa`, `storage/local`, `storage/s3`, `notification/slack`,
  `notification/mattermost`, `document/excel`·`docx` 같은 패키지로 나눈다.
- `core`에는 `FileStorage`, `ChatNotifier` 같은 순수 계약과 타입만 둔다. 구현체는
  `infrastructure`에 두고, 실행 앱의 업무 코드는 계약만 주입받는다. local/S3나 Slack/Mattermost의 선택은
  Spring profile·설정 조건으로 구현체 하나만 등록해 해결한다.
- 특정 SDK가 무겁거나 일부 실행 앱만 필요해 실제 의존성 분리의 이득이 생길 때에만
  `infrastructure` 하위 패키지를 별도 Gradle 모듈로 추출한다. 지금은 학습 복잡도를 늘리는 선분리를 하지 않는다.

## 2026-09-01 — HTTP 공통 관심사 패키지 정리

- `TokenCipher`는 암복호화 구현 외에 인증 기능의 Spring Component가 더 들어갈 수 있다는 판단으로
  `hjj.authentication.component`에 둔다. `crypto`보다 현재 책임을 과도하게 고정하지 않는 이름이다.
- 여러 엔드포인트에 적용하는 `AuthInterceptor`, 앞으로의 Servlet Filter, `@ControllerAdvice` 기반 오류 처리,
  이를 등록하는 `WebMvcConfig`는 HTTP 요청/응답 흐름의 공통 관심사로 묶는다.
  최종 위치는 `hjj.web/{error,interceptor,filter,config}`다.
- `Filter`·`Interceptor`·예외 처리기는 Spring 내부 단계가 서로 다르므로 `middleware`라는 기술 용어로
  뭉뚱그리지 않는다. 대신 모두 HTTP 경계를 다룬다는 의미의 `web`을 최상위 이름으로 택했다.

## 2026-09-01 — 공용 agent 메모리 구조를 .ai로 이관

- 저장소 전역의 작업 메모리·프로젝트 문서 canonical source를 기존 도구 전용 디렉터리에서 `.ai/`로 이관했다.
